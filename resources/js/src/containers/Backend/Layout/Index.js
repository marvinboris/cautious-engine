...
class Layout extends Component {
    ...
    render() {
        ...
        footerContent = <>
            {copyright} {new Date().getFullYear()} <span className={`text-700 text-${dark ? "blue" : "greenblue"}`}>{app_name}</span>. {all_rights} <img src={company_logo} style={{ height: 47 }} />.
        </>;
            ...
            <main className={`bg-${dark ? 'darkblue' : 'white'} position-relative min-vh-100 pb-4 pb-sm-5`}>
                <div className="toolbar-height row m-0 align-items-stretch border-bottom border-light sticky-top bg-white">
                    ...
                    <div className="col-4 border-left border-light d-flex align-items-center pl-3 pl-md-4 pl-xxl-5 pr-2 pr-md-3 pr-xxl-4">
                        <div className="mr-3 text-16 text-md-20 text-xxl-24 text-blue">
                ...
                <footer className={`position-absolute d-none d-sm-block w-100 py-3 px-4 bg-${dark ? "grayblue" : "green-15"} text-secondary`} style={{ bottom: 0 }}>
                    {footerContent}
                </footer>
            </main>
        </div>;
